...
9. `shift-subtitle-lines` / `stretch-subtitle-lines`: move or scale a selected lyric line range and rebuild `lyrics.ass`.
10. `edit-subtitles`: update line-level timing/text and rebuild `lyrics.ass`.
11. `mux`: create the final KTV MKV with generated ASS lyrics.
...
Local imports record a size plus partial SHA-256 source fingerprint in `report.json`. The fingerprint is used for duplicate-source warnings without hashing the entire video every time.

Mux stages run FFprobe against the generated MKV and write an audit summary with video/audio/subtitle counts, default track indexes, stream titles, and warnings.
